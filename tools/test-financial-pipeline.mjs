...
new vm.Script(scripts[1], { filename:"index.html#reporting" }).runInContext(context);
...
const taskWorkspaceSection=html.match(/<section id="task-workspace"[\s\S]*?<\/section>/)?.[0]||"";
assert.match(taskWorkspaceSection,/Zahl liefern/);
assert.match(taskWorkspaceSection,/Kommentar/);
assert.match(taskWorkspaceSection,/Nachweis hochladen/);
assert.match(taskWorkspaceSection,/Unteraufgabe erstellen/);
assert.match(html,/parent_task_id/);
const taskMigration=fs.readFileSync(path.join(root,"supabase","migrations","202607200001_task_workspace.sql"),"utf8");
assert.match(taskMigration,/add column if not exists parent_task_id/);
assert.match(taskMigration,/create table if not exists public\.task_entries/);
assert.match(taskMigration,/enable row level security/);
assert.match(taskMigration,/tasks_assignee_create_subtasks/);
assert.match(taskMigration,/task-evidence/);
const index39Template=fs.readFileSync(path.join(root,"reporting","index39-template.html"),"utf8");
assert.equal((index39Template.match(/<section class="page(?: active)?" id="page\d+">/g)||[]).length,12,"all twelve index39 report pages must be present");
assert.ok((index39Template.match(/<input\b/g)||[]).length>1100,"the original index39 input model must not be reduced to summary chips");
for(const title of ["Mustertexte Geschäftsbericht HGB","Anlagespiegel HGB","Rückstellungsspiegel HGB","Bilanz HGB","Gewinn- und Verlustrechnung HGB"]){assert.match(index39Template,new RegExp(title));}
assert.match(html,/id="index39Factory"/);
assert.match(html,/reporting\/index39-template\.html/);
assert.match(html,/attachShadow/);
const originalApiFetch=context.apiFetch;
context.apiFetch=async path=>path.includes("task_entries")?[]:[];
await assert.rejects(()=>context.validateTaskCompletion("task-1"),/mindestens eine Antwort, Zahl oder ein Nachweis/);
context.apiFetch=async path=>path.includes("task_entries")?[{entry_type:"answer"}]:[{id:"sub-1",status:"in_progress"}];
await assert.rejects(()=>context.validateTaskCompletion("task-1"),/Unteraufgabe/);
context.apiFetch=async path=>path.includes("task_entries")?[{entry_type:"evidence"}]:[{id:"sub-1",status:"done"}];
assert.equal(await context.validateTaskCompletion("task-1"),true);
...
pd.masterData.financialYear=2026;
pd.masterData.priorYear=2025;
...
const index39Amounts=context.index39StatementAmounts();
assert.equal(index39Amounts.get("PL::umsatzerlose").current,185_000_000,"index39 GuV must aggregate all mapped detail targets into the visible revenue line");
assert.equal(index39Amounts.get("PL::umsatzerlose").prior,170_000_000);
assert.equal(index39Amounts.get("BS::immateriellevermogensgegenstande").current,6_000_000,"same-named P&L depreciation lines must not contaminate balance-sheet amounts");
assert.ok(index39Amounts.size>70,"index39 must expose detail and hierarchy amounts, not only leaf mappings");
const index39Model=context.index39FinancialModel();
assert.equal(index39Model.plResult.current,9_000_000,"signed HGB GuV lines must reconcile to current-year net income");
assert.equal(index39Model.plResult.prior,7_000_000,"signed HGB GuV lines must reconcile to prior-year net income");
assert.equal(index39Model.balance.net_income.current,9_000_000,"net income must be included in equity exactly once");
assert.equal(index39Model.equity.current,56_000_000);
assert.equal(index39Model.equity.prior,50_000_000);
assert.equal(index39Model.fixedAssets.current,82_000_000);
assert.equal(index39Model.fixedAssets.prior,77_000_000);
const plPair=label=>index39Model.plInputs[label];
assert.equal(plPair('1. Umsatzerlöse').current,185_000_000);
assert.equal(plPair('a) Aufwendungen für Roh-, Hilfs- und Betriebsstoffe und für bezogene Waren').current+plPair('b) Aufwendungen für bezogene Leistungen').current,-92_000_000);
assert.equal(plPair('a) Löhne und Gehälter').current+plPair('b) Soziale Abgaben und Aufwendungen für Altersversorgung und für Unterstützung').current,-46_000_000);
assert.equal(plPair('a) Auf immaterielle Vermögensgegenstände des Anlagevermögens und Sachanlagen').current,-8_000_000);
const pairSum=keys=>keys.reduce((sum,key)=>sum+(index39Model.balance[key]?.current||0),0);
assert.equal(pairSum(["immat_self","immat_acquired","goodwill","immat_prepay","land_buildings","machinery","bga","cip","shares_affiliates_fa","loans_affiliates","participations","loans_participations","securities_fa","other_loans","inventories_raw","inventories_wip","inventories_finished","inventories_prepay","trade_receivables","receivables_affiliates","receivables_participations","other_assets","shares_affiliates_ca","other_securities","cash","prepaid_expenses_asset","deferred_tax_asset","asset_offset_difference"]),160_000_000,"visible index39 asset lines must reconcile to the balance-sheet total");
assert.equal(pairSum(["share_capital","capital_reserve","legal_reserve","reserve_parent_shares","statutory_reserves","other_revenue_reserves","retained_earnings","net_income","pension_provisions","tax_provisions","other_provisions","bonds","bank_liabilities","customer_advances","trade_payables","bill_liabilities","liabilities_affiliates","liabilities_participations","other_liabilities","deferred_income","deferred_tax_liability"]),160_000_000,"visible index39 liability/equity lines must reconcile to the balance-sheet total");
const fixedAssetFacts=context.index39FixedAssetFacts();
assert.equal(fixedAssetFacts.reduce((sum,row)=>sum+row.grossCurrent-row.depCurrent,0),82_000_000,"fixed-asset mirror closing book values must reconcile to the balance sheet");
assert.equal(fixedAssetFacts.reduce((sum,row)=>sum+row.grossPrior-row.depPrior,0),77_000_000,"fixed-asset mirror opening book values must reconcile to prior-year balance sheet");
assert.equal(fixedAssetFacts.reduce((sum,row)=>sum-row.depreciationCurrent,0),-8_000_000,"fixed-asset mirror depreciation must reconcile to the P&L");
context.CSS={escape:value=>value};
const cashflow=context.index39RefreshCashFlow({getElementById:id=>id==='cfTable'?{querySelectorAll:()=>[]}:null,querySelector:()=>null,querySelectorAll:()=>[]},index39Model);
assert.equal(cashflow.fundStart.current,7_000_000,"cash-flow opening funds must come from the prior-year balance sheet");
assert.equal(cashflow.fundEnd.current,12_000_000,"cash-flow bridge must reconcile to current liquid funds");
assert.equal(cashflow.fundEnd.prior,7_000_000,"prior-year cash-flow bridge must reconcile even without prior-prior movement data");
assert.equal(context.index39ParameterValue("shared::geschaeftsjahr","hgb"),2026,"original index39 year placeholders must use imported master data");
const placeholderFacts=context.index39FinancialFacts(index39Model,{operating:{current:12},investing:{current:-8},financing:{current:1},fundEnd:{current:12_000_000}});
const placeholder=(label,text='')=>context.index39FinancialPlaceholder({dataset:{placeholder:label},closest:()=>({textContent:text})},placeholderFacts);
assert.equal(placeholder('Umsatzerlöse Berichtsjahr'),185_000_000,"management-report revenue must be wired to the P&L");
assert.equal(placeholder('Umsatzerlöse Vorjahr','Das Ergebnis betrug'),9_000_000,"the mislabeled original result placeholder must be wired to net income");
assert.equal(placeholder('Betrag 1','Cashflow aus laufender Geschäftstätigkeit'),12,"management-report cash-flow placeholders must be wired to the cash-flow statement");
assert.match(html,/index39FinancialPlaceholder/);
assert.match(html,/Cashflow aus laufender Geschäftstätigkeit/);
pd.reports.index39Inputs["page5::1::1"]="125000";
pd.reports.index39Parameters["hgb::test_parameter"]={value:"Testwert"};
context.computeParameters();
assert.equal(pd.reports.index39Inputs["page5::1::1"],"125000","mirror movements must survive parameter recalculation");
assert.equal(pd.reports.index39Parameters["hgb::test_parameter"].value,"Testwert","manual index39 placeholders must survive financial imports");
